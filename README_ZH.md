...
下面两种方式使用同一个独立 demo。将以下源码保存为 `demo.cpp`：
...
使用 CMake 构建时，在 `demo.cpp` 同级目录将以下内容保存为 `CMakeLists.txt`：
...
该方式适用于已安装 RISC-V 交叉工具链的开发主机。解压发布的 `spine-runtime.xxx.tar.gz`，将 CMake 同时指向交叉工具链文件和解压后的 SDK，然后编译 demo：
...
在运行 Bianbu 的 K3 板子上安装 SpineRuntime 和原生编译工具。请以 `root` 执行安装命令，或在命令前添加 `sudo`；如果 APT 软件包索引已经过期，请先刷新索引。
